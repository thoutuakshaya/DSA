...
s=int(input())  #initial experience
calc=[] #list of tuples of power and bonus
p=[] #list of power
b=[] #list of bonus
for i in range(n): 
    p.append(int(input()))
for i in range(n): 
    b.append(int(input())) 
for i in range(n):    
    calc.append((p[i],b[i]) )  #creating list of tuples of power and bonus
calc.sort()
c=0
for p,b in calc: #iterating through the list of tuples
    if s>=p: #checking if the experience is greater than or equal to the power of the monster
        s+=b #if yes, then add the bonus to the experience and increment the count of monsters defeated
        c=c+1 #incrementing the count of monsters defeated
    else:
        break #if the experience is less than the power of the monster, then break the loop
print(c) #printing the count of monsters defeated
